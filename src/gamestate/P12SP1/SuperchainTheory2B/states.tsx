...
export type SuperchainTheory2bGameState =
  | InitialState
  | Paradeigma
  | Explosion1
  | Parthenos
...
export const getMechanic = (
  state: SuperchainTheory2bGameState,
  players: Player[]
): Mechanic<Player> => {
  if (state.stage === "Explosion1") {
    return composeMechanics([
      getSuperChainDangerPuddles(
        ["Circle"],
        point(0.5, state.first.north ? 0.375 : 0.625),
        players
      ),
      getSuperChainDangerPuddles(
        ["Donut"],
        point(0.5, state.first.north ? 0.625 : 0.375),
        players
      ),
    ]);
  }
  if (state.stage === "Explosion2") {
    return composeMechanics([
      getSuperChainDangerPuddles(
        ["Circle"],
        point(state.second.side === "East" ? 0.625 : 0.375, 0.5),
        players
      ),
      getSuperChainDangerPuddles(
        [state.second.type],
        point(state.second.side === "East" ? 0.375 : 0.625, 0.5),
        players
      ),
    ]);
  }
  if (state.stage === "Explosion3") {
    return composeMechanics([
      getSuperChainDangerPuddles(
        ["Circle"],
        point(0.5, state.third.north ? 0.25 : 0.75),
        players
      ),
      getSuperChainDangerPuddles(
        ["Circle", "Protean"],
        point(0.5, state.third.north ? 0.75 : 0.25),
        players
      ),
    ]);
  }
  ...
  if (state.stage === "AddLines") {
    return composeMechanics(
      [0.2, 0.6].map((x) =>
        lineMechanic(
          point(state.second.side === "East" ? x : 1 - x, 0),
          0,
          0.25,
          SimpleKillProfile
        )
...
export const getTargetSpot = (
  ...
  if (state.stage === "Parthenos") {
    ...
    switch (player.designation) {
      case "MT":
        off =
          state.second.type === "Pair" ? vector(0.7, -0.7) : vector(0.9, -0.4);
        break;
      case "M1":
        off =
          state.second.type === "Pair" ? vector(0.7, -0.7) : vector(0.4, -0.9);
        break;
      case "OT":
        off =
          state.second.type === "Pair" ? vector(0.7, 0.7) : vector(0.9, 0.4);
    ...
    return point(state.second.side === "East" ? 0.375 : 0.625, 0.5).translate(
      off.scale(0.05, 0.05)
    );
  }
  if (state.stage === "Explosion2") {
  ...
  if (state.stage === "AddLines") {
    ...
    return point(0.5, state.third.north ? 0.75 : 0.25).translate(
      off.scale(0.22, 0.22)
    );
  }
